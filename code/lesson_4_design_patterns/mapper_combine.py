...
"12345"  "A"  "11"  "3"  "4"  "1"
"12345"  "B"   "6336" "Unit 1: Same Value Q"  "cs101 value same"  "question"  "\N"  "\N"  "2012-02-25 08:09:06.787181+00"   
	...
	for line in reader:
		if len(line)==5:
			...
			if user_ptr_id=="user_ptr_id":
				continue
			entry=[user_ptr_id,aString,reputation,gold,silver,bronze]
			writer.writerow(entry)
		if len(line)==19:
			id_m,title,tagnames,author_id,body,node_type,parent_id,abs_parent_id,added_at,score,state_string,last_edited_id,last_activity_by_id,last_activity_at,active_revision_id,extra,extra_ref_id,extra_count,marked=line
			if id_m=="id":
				continue
			entry=[author_id,bString,id_m,title,tagnames,node_type,parent_id,abs_parent_id,added_at,score]
			writer.writerow(entry)
